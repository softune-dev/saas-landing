...
import { getHomeFaqs } from "@/lib/home-faq-data";

export function Faq({ locale = "en" }: { locale?: "en" | "bn" }) {
  ...
  const renderFaqCard = (item: { q: string; a: string }, index: number) => {
    ...
    return (
      ...
      >
        ...
        >
          <span
            className={`text-[16px] md:text-[18px] font-semibold tracking-tight transition-colors ${
          ...
          <div
            className={`flex size-8 md:size-10 shrink-0 items-center justify-center rounded-full transition-colors duration-300 ${
              isOpen
            ...
            {isBn ? (
              <>
                ...
                <span className="relative ml-0.5 inline-block px-2.5 py-0.5 sm:ml-1 sm:whitespace-nowrap sm:px-4">
                  ...
                  <em className="relative not-italic text-white">প্রশ্ন জানা দরকার</em>
                </span>
              </>
            ) : (
              <>
